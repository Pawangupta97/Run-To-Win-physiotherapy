...
export const TestimonialsSection: React.FC<TestimonialsSectionProps> = ({ onOpenBooking }) => {
  ...
  const [allTestimonials, setAllTestimonials] = useState<Testimonial[]>(() => getAllCombinedTestimonials());

  // Load any user-submitted reviews and listen for Google sync updates
  useEffect(() => {
    const updateTestimonials = () => {
      try {
        const combined = getAllCombinedTestimonials();
        ...
        if (stored) {
          ...
          if (Array.isArray(userReviews) && userReviews.length > 0) {
            ...
            const filteredBase = combined.filter((t) => !userIds.has(t.id));
            setAllTestimonials([...userReviews, ...filteredBase]);
            return;
          }
        }
    ...
    updateTestimonials();
    window.addEventListener('rtw-reviews-synced', updateTestimonials);
    return () => window.removeEventListener('rtw-reviews-synced', updateTestimonials);
  }, []);

  const handleSyncGoogle = async () => {
    setIsSyncing(true);
    try {
      await fetchGoogleCustomerReviews();
      setAllTestimonials(getAllCombinedTestimonials());
    } catch (e) {
  ...
  const tags = ['All', 'Google Reviews', 'Spine & Sciatica', 'Home Care Post-Op', 'Sports Injury', 'Frozen Shoulder', 'Neuro Stroke Rehab'];

  const filteredTestimonials = selectedTag === 'All'
    ? allTestimonials
    : selectedTag === 'Google Reviews'
    ? allTestimonials.filter((t) => t.source === 'google_business' || t.id.startsWith('gmb-'))
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-10 p-5 sm:p-6 rounded-3xl bg-white border border-slate-200/90 shadow-lg shadow-blue-500/5 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2.5 w-full md:w-auto">
              ...
              <button
                ...
                disabled={isSyncing}
                className="px-4 py-2.5 rounded-full bg-white hover:bg-slate-50 text-slate-800 border border-slate-300 text-xs font-bold transition flex items-center space-x-1.5 shadow-sm hover:border-blue-400 cursor-pointer disabled:opacity-60"
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-full bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition-all shadow-md shadow-blue-500/20 flex items-center space-x-1.5 hover:scale-105 cursor-pointer"
              >
          ...
          <div className="flex items-center justify-start sm:justify-center overflow-x-auto pb-3 mb-8 gap-2 no-scrollbar">
            {tags.map((tag) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all duration-150 border flex items-center space-x-1.5 ${
                  selectedTag === tag
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTestimonials.map((item) => {
              const isGoogle = item.source === 'google_business' || item.id.startsWith('gmb-');
              return (
                <div
                  key={item.id}
                  className={`bg-white rounded-3xl border ${
                    item.id.startsWith('user-rev-') 
                      ...
                      : isGoogle
                      ? 'border-blue-200 shadow-md shadow-blue-500/5 ring-1 ring-blue-500/20'
                      : 'border-slate-200'
                ...
                >
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Patient Condition & Profile Info */}
                    ...
                    {item.reply && (
                      <div className="p-3 rounded-2xl bg-blue-50/80 border border-blue-100 text-[11px] text-slate-800 space-y-1">
                        ...
                        <p className="text-slate-600 italic">"{item.reply}"</p>
                      </div>
                    )}
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-1 text-emerald-600 text-xs font-semibold">
                      ...
                      <span className="text-[10px]">{isGoogle ? 'Google Review' : 'Verified'}</span>
                    </div>
                  </div>
